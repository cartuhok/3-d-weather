    ...
    <group position={position}>
      ...
      {!isFullscreen && (
        <>
          ...
          {/* Overlay text with better spacing */}
          {/* Day at top - left aligned */}
          <Text
          ...
          {/* Condition at bottom - left aligned */}
          <Text
            position={[-0.8, -1.0, 0.1]}
            fontSize={0.13}
            color="#FFFFFF"
...
const ForecastPortals = ({ weatherData, isLoading, onPortalStateChange }) => {
  ...
  // Calculate mobile-responsive scaling
  const isMobile = viewport.width < 6; // Adjust threshold as needed
  const scale = isMobile ? 0.7 : 1;
  const spacing = isMobile ? 2.2 : 3; // Tighter spacing on mobile

  // Smooth camera animation only when transitioning to portal mode
  useFrame((state, delta) => {
    ...
    <>
      <group position={[0, -0.5, 0]} scale={[scale, scale, scale]}>
        {forecastDays.map((day, index) => (
          <ForecastPortal
            key={day.date}
            position={[-spacing + index * spacing, 0, 0]} // Responsive spacing
            dayData={day}
            index={index}
