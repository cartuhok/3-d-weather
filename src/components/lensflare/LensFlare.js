...
function UltimateLensFlare({
  ...
  animated = true,
  anamorphic = false,
  colorGain = new Color(255, 165, 0), // Orange sun color
  ...
  useFrame(({ scene, mouse, camera, delta }) => {
    if (lensRef.current) {
      if (followMouse) {
        ...
        // Hide flare if sun is behind camera
        ...
        if (intersects.length > 0) {
          ...
          // Check if it's our sun's invisible trigger sphere
          if (firstIntersect.object.userData && firstIntersect.object.userData.lensflare === 'no-occlusion') {
            easing.damp(lensRef.current.uniforms.get('opacity'), 'value', 1.0, 0.07, delta)
          } else {
            // Check for transparent/transmissive materials
